...
class MosesPacket {
    ...
    /**
     * Constructor for the Packet Class. Takes an array of bytes (must be 
     * in the form of a Moses Packet) and parses it into individual fields.
     * 
...
//        }
    ...
    /**
     * Constructor for the Packet Class. Takes the given parameters  
     * and builds a Moses-formatted byte array packet.
     * 
     * @param in_type The packet's main type. (ie Packet.TYPE_MAIN_TIMER, Packet.TYPE_MAIN_...)
    ...
    public MosesPacket(char in_type, String in_subType, byte[] in_data)
    {
        ...
        packetBytes = new byte[15 + packetDataLength];
        
        /* Put in the delimiters. and EOF char */
        packetBytes[0] = (byte)((byte)StartDelimiter | 0x80);
        packetBytes[packetBytes.length -1] = StopDelimiter;
        ...
        packetBytes[1] = (byte)timeStampStr.charAt(0);
        packetBytes[2] = (byte)timeStampStr.charAt(1);
        packetBytes[3] = (byte)timeStampStr.charAt(2);
        packetBytes[4] = (byte)timeStampStr.charAt(3);
        packetBytes[5] = (byte)timeStampStr.charAt(4);
        packetBytes[6] = (byte)timeStampStr.charAt(5);
        
        /* Put in the packet type and sub type. */
        packetBytes[7] = (byte)in_type;
        packetBytes[8] = (byte)in_subType.charAt(0);
        packetBytes[9] = (byte)in_subType.charAt(1);
        packetBytes[10] = (byte)in_subType.charAt(2);
        
        /* Put in the data field's length */
        String lengthStr = String.format("%02x", packetDataLength);
        packetBytes[11] = (byte)lengthStr.charAt(0);
        packetBytes[12] = (byte)lengthStr.charAt(1);
        
        /* Put in the data */
        System.arraycopy(in_data, 0, packetBytes, 13, packetDataLength);
        ...
        /* Calculate CheckSum */
        packetBytes[packetBytes.length -2] = generateChecksum();
    ...
    /**
     * Generates the checksum for a packet.
     * Should only be called once packetBytes has been filled with 
     * 
